...
if ( isset( $_POST['frm_gNews_display'] ) && $_POST['frm_gNews_display'] == 'yes' ) {
	...
	$sSql = $wpdb->prepare("SELECT COUNT(*) AS `count` 
						 	 FROM " . WP_G_NEWS_ANNOUNCEMENT . "
						 	 WHERE `gNews_id` = %d", array( $did )
	                      );
	$result = '0';
	$result = $wpdb->get_var( $sSql );
	
	if ( $result != '1') {
		?><div class="error fade"><p><strong><?php _e( 'Oops, selected details doesnt exist.', 'news-announcement-scroll' ); ?></strong></p></div><?php
	} else if ( isset( $_GET['ac'] ) && $_GET['ac'] == 'del' && isset( $_GET['did'] ) && $_GET['did'] != '' ) {       // Form submitted, check the action
...
<div class="wrap">
    ...
    <h2><?php _e( 'News Announcement Scroll', 'news-announcement-scroll' ); ?>
	<a class="add-new-h2" href="<?php echo WP_G_NEWS_ADMIN_URL; ?>&amp;ac=add"><?php _e( 'Add New', 'news-announcement-scroll' ); ?></a></h2>
    <div class="tool-box">
	    <?php
			...
			if ( count( $myData ) > 0 ) {
				foreach ($myData as $data) {
					...
					<tr class="<?php if ( $i&1 ) { echo 'alternate'; } else { echo ''; }?>">
						...
						<td><?php echo esc_html( stripslashes( chunk_split( $data['gnews_redirect_link'],20 ) ) ); ?></td>
				...
				?><tr><td colspan="7" align="center"><?php _e( 'No records available.', 'news-announcement-scroll' ); ?></td></tr><?php
			}
			?>
		</tbody>
        </table>
	  ...
	  <div class="tablenav">
	  <h2>
	  <a class="button add-new-h2" href="<?php echo WP_G_NEWS_ADMIN_URL; ?>&amp;ac=add"><?php _e( 'Add New', 'news-announcement-scroll' ); ?></a>
	  <a class="button add-new-h2" href="<?php echo WP_G_NEWS_ADMIN_URL; ?>&amp;ac=set"><?php _e( 'Widget Settings', 'news-announcement-scroll' ); ?></a>
	  <a class="button add-new-h2" target="_blank" href="<?php echo WP_G_NEWS_HELP; ?>"><?php _e( 'Help', 'news-announcement-scroll' ); ?></a>
	  </h2>
	  </div>
	  <h3><?php _e('Shortcode Details :', 'news-announcement-scroll'); ?></h3>
	<ol>
		<li><?php _e('Shortcode for version 2.0 to 5.0 : [NEWS:TYPE=widget]', 'news-announcement-scroll'); ?> </li>
		<li><?php _e('Shortcode for version 6.0 to 8.7.1 : [news-announcement type="widget"]', 'news-announcement-scroll'); ?> </li>
		<li><?php _e('Shortcode for version 8.8 onwards: [news-announcement group="sample"]', 'news-announcement-scroll'); ?> </li>
	</ol>
	  <p class="description">
